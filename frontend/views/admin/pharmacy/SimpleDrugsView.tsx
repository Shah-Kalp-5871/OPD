...
import { pharmacyAdminApi, SimpleDrug } from '@/lib/api/pharmacy';
import { toast } from 'sonner';
import AdminLayout from '@/views/layouts/AdminLayout';

export default function SimpleDrugsView() {
  const [drugs, setDrugs] = useState<SimpleDrug[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [search, setSearch] = useState('');
  
  const [form, setForm] = useState<Partial<SimpleDrug>>({
    drugName: '',
    category: '',
    unitPrice: 0,
    stockQuantity: 0,
  ...
  const loadDrugs = async () => {
    try {
      setLoading(true);
      const data = await pharmacyAdminApi.getSimpleDrugs();
      setDrugs(data);
    } catch (e: any) {
      toast.error('Failed to load simple drugs');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await pharmacyAdminApi.updateSimpleDrug(editingId, payload);
        toast.success('Drug updated successfully');
      } else {
        await pharmacyAdminApi.createSimpleDrug(payload);
  ...
  const handleEdit = (drug: SimpleDrug) => {
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to disable this drug?')) {
      try {
        await pharmacyAdminApi.deleteSimpleDrug(id);
  ...
  const handleToggle = async (drug: SimpleDrug) => {
    try {
      await pharmacyAdminApi.updateSimpleDrug(drug.id, { isActive: !drug.isActive });
  ...
  return (
    <AdminLayout>
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Simple Drugs Master</h1>
        <button
          onClick={() => {
            setEditingId(null);
            setForm({
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-4 border-b border-slate-100">
          <div className="relative max-w-md">
            ...
            <input
              type="text"
              placeholder="Search simple drugs..."
              value={search}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 text-slate-500 text-xs uppercase tracking-wider">
                ...
                <th className="p-4 font-bold border-b border-slate-100">Stock Qty</th>
              ...
              {loading ? (
                ...
                filteredDrugs.map((drug) => (
                  <tr key={drug.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-4 text-sm text-slate-600">{drug.category || '-'}</td>
                    <td className="p-4 text-sm text-slate-600">₹{drug.unitPrice}</td>
                    <td className="p-4 text-sm text-slate-600">
                      <span className={`px-2 py-1 rounded font-medium ${drug.stockQuantity <= drug.lowStockLimit ? 'bg-orange-100 text-orange-700' : 'bg-slate-100 text-slate-700'}`}>
                        {drug.stockQuantity}
                      </span>
                    </td>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              <h2 className="text-xl font-bold text-slate-800">
                {editingId ? 'Edit Simple Drug' : 'Add New Simple Drug'}
              </h2>
            </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setForm({ ...form, stockQuantity: e.target.value === '' ? 0 : parseInt(e.target.value) })}
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-800 outline-none focus:border-indigo-400 focus:bg-white transition-all"
                  />
                </div>
              </div>

...
const PatientRegistrationView = () => {
  ...
  const onSubmit = async (data: PatientFormValues) => {
    ...
    try {
      ...
      router.push(`/doctor/appointments?patientId=${response.data.id}`);
    } catch (error: any) {
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex bg-slate-100 p-1.5 rounded-2xl">
              ...
              <button
                ...
                className={`px-5 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'MR' ? 'bg-white text-[#0d6282] shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
              >

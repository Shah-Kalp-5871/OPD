  ...
  if (loading) {
    return (
      <div className="h-[40vh] flex flex-col items-center justify-center gap-4 bg-white rounded-3xl border border-slate-100 shadow-sm">
        <Loader2 className="w-10 h-10 text-[#0d6282] animate-spin" />
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-6 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
          ...
          <div className="bg-sky-100/50 text-sky-700 px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border border-sky-100">
            System Verified
          </div>
        </div>
        <div className="p-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-x-12 gap-y-8">
            {[
              ...
              <div key={idx} className="group">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 group-hover:text-[#0d6282] transition-colors">{field.label}</p>
            ...
            <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
              {templates.map((template) => (
                <button
                  ...
                  className={`w-full p-4 rounded-2xl border-2 transition-all text-left group ${selectedTemplate?.id === template.id ? 'bg-sky-50 border-sky-500' : 'bg-white border-slate-100 hover:border-slate-200'}`}
                >
                  <p className={`text-[10px] font-black uppercase tracking-widest ${selectedTemplate?.id === template.id ? 'text-sky-700' : 'text-slate-500'}`}>
                    {template.name}
                  </p>
                </button>
              ))}
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-100 space-y-4">
              ...
              <div className="flex flex-col md:flex-row gap-4 pt-2">
                ...
                <button 
                  ...
                  className="flex-1 py-4 bg-[#0d6282] text-white rounded-xl text-[11px] font-black uppercase tracking-[0.2em] hover:bg-[#0a4b63] transition-all shadow-md disabled:opacity-50 flex items-center justify-center gap-3"
                >

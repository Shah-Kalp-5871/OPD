      ...
      <div className="bg-white border border-slate-200/60 rounded-3xl overflow-hidden shadow-sm">
        ...
        <div className="px-8 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <h3 className="text-xs font-black text-slate-800 uppercase tracking-widest flex items-center gap-3">
            <History className="w-4 h-4 text-[#0d6282]" />
            Complaints & Vitals
          </h3>
          <div className="flex items-center gap-3">
            ...
            {sortedCases.findIndex((c: any) => c.id === selectedCaseId) + 1 < sortedCases.length && (
              <button onClick={handleSameAsPrevious} className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold text-[#0d6282] bg-sky-50 hover:bg-sky-100 border border-sky-200 rounded-lg transition-colors">
                <Copy className="w-3 h-3" /> Same as Previous
              </button>
            )}
            <button onClick={() => router.push(`/doctor/patients/${patient.id}/history`)} className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold text-slate-600 bg-white hover:bg-slate-50 border border-slate-200 rounded-lg transition-colors shadow-sm">
        ...
        <div className="p-6 md:p-8 space-y-8">
          ...
          <section>
            <h4 className="text-sm font-semibold text-slate-800 mb-6 border-b border-slate-100 pb-2 flex items-center gap-2">
              <Activity className="w-4 h-4 text-[#0d6282]"/> Vitals
            </h4>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3 items-end">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">Temp (°F)</label>
                <input type="text" value={vitals.temp} onChange={e => setVitals({...vitals, temp: e.target.value.replace(/[^0-9.]/g, '')})} className="w-full px-3 py-2.5 text-sm font-bold bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500 outline-none text-center" placeholder="98.6" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">Pulse (BPM)</label>
                <input type="text" value={vitals.pulse} onChange={e => setVitals({...vitals, pulse: e.target.value.replace(/[^0-9]/g, '')})} className="w-full px-3 py-2.5 text-sm font-bold bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500 outline-none text-center" placeholder="72" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">Sys (mmHg)</label>
                <input type="text" value={vitals.bpSys} onChange={e => setVitals({...vitals, bpSys: e.target.value.replace(/[^0-9]/g, '')})} className="w-full px-3 py-2.5 text-sm font-bold bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500 outline-none text-center" placeholder="120" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">Dia (mmHg)</label>
                <input type="text" value={vitals.bpDia} onChange={e => setVitals({...vitals, bpDia: e.target.value.replace(/[^0-9]/g, '')})} className="w-full px-3 py-2.5 text-sm font-bold bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500 outline-none text-center" placeholder="80" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">Weight (kg)</label>
                <input type="text" value={vitals.weight} onChange={e => setVitals({...vitals, weight: e.target.value.replace(/[^0-9.]/g, '')})} className="w-full px-3 py-2.5 text-sm font-bold bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500 outline-none text-center" placeholder="70" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">Height (cm)</label>
                <input type="text" value={vitals.height} onChange={e => setVitals({...vitals, height: e.target.value.replace(/[^0-9.]/g, '')})} className="w-full px-3 py-2.5 text-sm font-bold bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500 outline-none text-center" placeholder="170" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">SpO2 (%)</label>
                <input type="text" value={vitals.spo2} onChange={e => setVitals({...vitals, spo2: e.target.value.replace(/[^0-9]/g, '')})} className="w-full px-3 py-2.5 text-sm font-bold bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500 outline-none text-center" placeholder="98" />
              </div>
          ...
          <div className="pt-5 flex justify-end gap-3">
            ...
            <button 
              ...
              className={`flex items-center gap-2 px-8 py-3.5 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all ${
                (!isSaving && selectedCaseId) ? 'bg-[#0d6282] text-white hover:bg-[#0a4b63] shadow-md shadow-sky-500/20 hover:shadow-lg hover:shadow-sky-500/30 hover:-translate-y-0.5' : 'bg-slate-100 text-slate-400 cursor-not-allowed'
              }`}
            >

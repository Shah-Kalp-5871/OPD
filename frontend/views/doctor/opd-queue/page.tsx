...
const OpdQueueView = () => {
  ...
  const [selectedDoctor, setSelectedDoctor] = useState<string>(user?.id || 'all');
  ...
  const getStatusBadgeString = (status: string) => {
    switch (status) {
      ...
      case 'IN_SESSION':
        return `<div class="px-2 py-1 bg-sky-100 text-sky-700 rounded text-[10px] font-black uppercase tracking-widest border border-sky-200 inline-block animate-pulse">IN PROGRESS</div>`;
      case 'COMPLETED':
  ...
  const SortIcon = ({ columnKey }: { columnKey: string }) => {
    ...
    return <span className="text-[#0d6282] ml-1 inline-block">{sortConfig.direction === 'asc' ? '↑' : '↓'}</span>;
  };
      ...
      <div className="space-y-6 pb-20 w-full mx-auto">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between bg-white p-3 lg:p-4 rounded-[1.5rem] border border-slate-200 shadow-sm gap-4">
          <div className="flex items-center gap-3">
             <div className="w-10 h-10 bg-[#0d6282] rounded-xl flex items-center justify-center shadow-lg shadow-sky-200">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 px-2">
          {/* Tabs removed: Merge into single table */}
              ...
              return (
                <button 
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border transition-all ${
                    isSelected 
                      ? 'bg-sky-50 border-sky-200 text-orange-800 shadow-sm' 
                      : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'
                  }`}
                >
                  {isSelected ? <CheckSquare className="w-3.5 h-3.5 text-[#0d6282]" /> : <Square className="w-3.5 h-3.5 text-slate-400" />}
                 ...
                 {currentQueueData.length === 0 ? (
                   ...
                   currentQueueData.map((entry, idx) => {
                      ...
                      return (
                        <tr 
                          key={entry.id || idx} 
                          onClick={() => !isInSession && entry.patient?.id && router.push(`/doctor/patients/${entry.patient.id}/`)}
                        ...
                        >
                         <td className="px-2 lg:px-3 py-2.5 whitespace-nowrap text-[11px] font-black text-slate-800 border-r border-slate-50">
                           <div className="flex items-center gap-1.5 justify-center">
                             {isCalling && <Bell className="w-3.5 h-3.5 text-blue-700 animate-bounce" />}
                             <span>{entry.isAppointment ? '--' : (entry.case?.caseNumber || entry.tokenDisplay)}</span>
                           </div>
                         </td>
                         ...
                         <td className="px-2 lg:px-3 py-2.5 whitespace-nowrap border-r border-slate-50">
                            <div className="flex flex-col items-center gap-1">
                              <div className={`text-[12px] font-black uppercase tracking-wider ${isInSession ? 'text-[#0d6282] animate-pulse' : isCalling ? 'text-blue-600 animate-pulse' : 'text-slate-900'} flex items-center justify-center gap-1.5`}>
                               ...
                               return (
                                 <div className="flex flex-col gap-1 items-center justify-center w-[80px] mx-auto">
                                   <div className="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden flex">
                                     <div 
                                       className={`h-full ${isFullyPaid ? 'bg-green-500' : isPartiallyPaid ? 'bg-sky-500' : 'bg-transparent'}`} 
                                   ...
                                   <div className={`text-[9px] font-black tracking-widest uppercase ${isFullyPaid ? 'text-green-600' : isPartiallyPaid ? 'text-[#0d6282]' : 'text-red-600'}`}>

    ...
    <ReceptionLayout>
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          <button 
            onClick={() => router.push(`/reception/patients/${patientId}`)}
               ...
               {timelineEvents.map((event: any, index: number) => {
                 ...
                 return (
                   <div key={event.id} className="relative pl-8 md:pl-10">
                     ...
                     {/* Timeline Dot */}
                     <div className={`absolute top-1 left-0 w-8 h-8 md:w-10 md:h-10 rounded-full border-4 border-white flex items-center justify-center z-10 shadow-sm ${isCancel ? 'bg-rose-50 text-rose-600' : 'bg-amber-50 text-amber-600'}`}>
                       {isCancel ? <XCircle className="w-3.5 h-3.5 md:w-4 md:h-4" /> : <RefreshCw className="w-3.5 h-3.5 md:w-4 md:h-4" />}
                     </div>

                     <div className={`bg-white border rounded-2xl p-5 md:p-6 shadow-sm ${isCancel ? 'border-rose-100' : 'border-amber-100'}`}>
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                           <div>
                             <div className="flex items-center gap-3 mb-1">
                                <span className={`text-xs font-bold px-2.5 py-1 rounded-md uppercase tracking-wider ${isCancel ? 'bg-rose-100 text-rose-700' : 'bg-amber-100 text-amber-700'}`}>
                        ...
                        {event.remarks && (
                          <div className={`mt-4 p-3 rounded-xl border text-sm ${isCancel ? 'bg-rose-50 border-rose-100 text-rose-800' : 'bg-amber-50 border-amber-100 text-amber-800'}`}>

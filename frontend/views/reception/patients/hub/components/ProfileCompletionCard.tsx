  ...
  return (
    <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-8 space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          {patient.profile?.address ? <CheckCircle2 className="w-5 h-5 text-teal-500" /> : <Circle className="w-5 h-5 text-slate-200" />}
          <span className={`text-xs font-bold ${patient.profile?.address ? 'text-slate-600' : 'text-slate-400'}`}>Address Details</span>
        </div>
        <div className="flex items-center gap-3">
          {patient.profile?.occupation ? <CheckCircle2 className="w-5 h-5 text-teal-500" /> : <Circle className="w-5 h-5 text-slate-200" />}
          <span className={`text-xs font-bold ${patient.profile?.occupation ? 'text-slate-600' : 'text-slate-400'}`}>Occupation & Notes</span>
        </div>
        <div className="flex items-center gap-3">
          {patient.profile?.emergencyContact ? <CheckCircle2 className="w-5 h-5 text-teal-500" /> : <Circle className="w-5 h-5 text-slate-200" />}

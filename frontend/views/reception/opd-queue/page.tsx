           ...
           {isLoading ? (
             <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/80 backdrop-blur-sm z-10 space-y-6">
                ...
                <div className="flex flex-col items-center gap-2">
                   ...
                   <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Establishing Real-time Link...</p>
                </div>
             </div>
                        ...
                        <tr key={entry.id} className={`group transition-all duration-300 ${entry.status === 'IN_SESSION' ? 'bg-teal-50/20' : 'hover:bg-slate-50/50'}`}>
                           ...
                           <td className="px-8 py-7">
                              <div className="flex flex-col gap-1">
                                 <div className="flex items-center gap-2">
                                    ...
                                    {entry.case.visitType === 'NEW' && (
                                       <span className="px-1.5 py-0.5 bg-indigo-100 text-indigo-700 rounded text-[7px] font-black uppercase tracking-tighter border border-indigo-200">NEW</span>
                                    )}
                                 </div>
                                 <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-bold text-slate-500 uppercase">{entry.patient.gender}</span>
                                 </div>
                              </div>
                           </td>
                           <td className="px-8 py-7">
                              <div className="flex flex-col gap-2">
                                 <div className="flex items-center gap-2">
                                    <span className={`text-[9px] font-black px-2.5 py-1 rounded-full border uppercase tracking-widest flex items-center gap-1.5 ${getPriorityColor(entry.priority)}`}>
                                       <div className={`w-1.5 h-1.5 rounded-full ${entry.priority === 'EMERGENCY' ? 'bg-rose-500 animate-ping' : entry.priority === 'URGENT' ? 'bg-amber-500' : 'bg-slate-400'}`}></div>
                                       {entry.priority}
                                    </span>
                                 </div>
                           ...
                           <td className="px-8 py-7">
                              <div className="flex flex-col gap-1.5">
                                 ...
                                 <span className="text-[8px] font-black text-slate-400 uppercase tracking-tighter">Case No: {entry.case.caseNumber}</span>
                              </div>
                           </td>
                           <td className="px-8 py-7">
                              ...
                              <div className="flex items-center justify-end gap-3">
                                 {entry.status === 'WAITING' && (
                                   <button 
                                     ...
                                     className="group/btn relative bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-md shadow-blue-100 flex items-center gap-2 active:scale-95 disabled:opacity-50"
                                   >
                                      <PhoneCall className="w-3.5 h-3.5 group-hover/btn:animate-bounce" />
                                      Call Patient
                                   </button>
                                 )}
                                 ...
                                 {entry.status === 'CALLING' && (
                                   <div className="flex items-center gap-2 bg-slate-50 p-1.5 rounded-2xl border border-slate-100 shadow-inner">
                                      <button 
                                        onClick={() => handleUpdateStatus(entry.id, 'WAITING', 'RE_QUEUE')}
                                        disabled={isActionLoading === entry.id}
                                        className="bg-white text-slate-600 px-4 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-slate-100 transition-all border border-slate-200"
                                      >
                                         Retry
                                      </button>
                                      <button 
                                        onClick={() => handleUpdateStatus(entry.id, 'NO_RESPONSE', 'MARK_ABSENT')}
                                        disabled={isActionLoading === entry.id}
                                        className="bg-amber-500 text-white px-4 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-amber-600 transition-all shadow-md shadow-amber-100"
                                      >
                                         Absent
                                      </button>
                                   </div>
                                 ...
                                 {(entry.status === 'NO_RESPONSE' || entry.status === 'CANCELLED') && (
                                   <button 
                                     onClick={() => handleUpdateStatus(entry.id, 'WAITING', 'RE_ACTIVATE')}
                                     disabled={isActionLoading === entry.id}
                                     className="bg-teal-50 text-teal-700 px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-teal-100 transition-all border border-teal-200"
                                   >
                                      Re-Activate
                                   </button>
                                 )}
